...
class FailureRateResultTest extends TestCase
{
    ...
    {
        $result = FailureRateResult::from('org/repo', 'alice', total: 10, failures: 2);
        $this->assertEqualsWithDelta(0.2, $result->rate, 1e-9);
    }

    public function testRateIsZeroWhenNoBuilds(): void
    {
        $result = FailureRateResult::from('org/repo', 'alice', total: 0, failures: 0);
        $this->assertSame(0.0, $result->rate);
    }

    public function testCarriesFields(): void
    {
        $result = FailureRateResult::from('org/repo', 'bob', total: 5, failures: 1);
        $this->assertSame('org/repo', $result->repoFullName);
